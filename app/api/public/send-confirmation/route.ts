 ...
 * Called by the marketing-page voice agent to send confirmation emails.
 * Rate-limited to 3 requests per minute per IP.
 */
export async function POST(req: NextRequest) {
    try {
        // Rate limit by IP (stricter than authenticated route)
        ...
        if (!body.requestNumber || !body.customerName || !body.customerEmail || !body.serviceType) {
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    `.trim();
      ...
      `,
        ...
        <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 2px solid #dc2626; border-radius: 12px; overflow: hidden;">
          ...
          <div style="padding: 24px;">
            <h2 style="margin-top: 0; color: #1f2937;">${escapeHtml(body.requestNumber)}</h2>
            <h3 style="color: #374151; border-bottom: 1px solid #e5e7eb; padding-bottom: 8px;">👤 Customer</h3>
            <table style="width: 100%; font-size: 14px; color: #374151;">
              <tr><td style="padding: 4px 0;"><strong>Name:</strong></td><td>${escapeHtml(body.customerName)}</td></tr>
              <tr><td style="padding: 4px 0;"><strong>Email:</strong></td><td><a href="mailto:${escapeHtml(body.customerEmail)}">${escapeHtml(body.customerEmail)}</a></td></tr>
              <tr><td style="padding: 4px 0;"><strong>Phone:</strong></td><td>${body.customerPhone ? escapeHtml(body.customerPhone) : "Not provided"}</td></tr>
              ${body.companyName ? `<tr><td style="padding: 4px 0;"><strong>Company:</strong></td><td>${escapeHtml(body.companyName)}</td></tr>` : ""}
            </table>
      ...
      `,
        ...
        console.error("Public email send error:", error);
        return NextResponse.json(
